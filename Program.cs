...
{
    List<Show> order = shows.OrderBy(s => s.Name).ToList();
    return order.Select(s => s.Name).ToList();
}
...
{
    return shows.Where(s => s.Name.StartsWith("&")).ToList();
}
...
{
    List<Show> notTheLowest = shows.OrderBy(s => s.ImdbRating).ToList();
    return notTheLowest.Skip(1).ToList();
}

// 11. Return the names of the shows that had fewer than 100 episodes.
static List<string> FewEpisodes(List<Show> shows)
{
    List<Show> Episodes = shows.Where(s => s.EpisodeCount < 100).ToList();
    List<string> Result = Episodes.Select(s => s.Name).ToList();
    return Result;
}
...
{
    List<Show> comedy = shows.Where(s => s.Genres.Contains("Comedy")).OrderBy(s => s.ImdbRating).ToList();
    return comedy.Select(s => s.Name).ToList();
}
...
{
    List<Show> multipleGenres = shows.Where(s => s.Genres.Count > 1).ToList();
    return multipleGenres.OrderBy(s => s.StartYear).ToList();
}

// 15. Return the show with the most episodes.
static Show MostEpisodes(List<Show> shows)
{
    int higherEpisodeCount = shows.Max(s => s.EpisodeCount);
    return shows.FirstOrDefault(s => s.EpisodeCount == higherEpisodeCount);
}
...
{
    List<Show> endingYear = shows.OrderBy(s => s.EndYear).ToList();
    return endingYear.FirstOrDefault(s => s.EndYear >= 2000);
}
...
{
    List<Show> ratings = shows.OrderByDescending(s => s.ImdbRating).ToList();
    return ratings.FirstOrDefault(s => s.Genres.Contains("Drama"));
}

// 18. Return all dramas except for the highest rated.
static List<Show> AllButBestDrama(List<Show> shows)
{
    List<Show> dramas = shows.Where(s => s.Genres.Contains("Drama")).OrderByDescending(p => p.ImdbRating).ToList();
    return dramas.Skip(1).ToList();
}

// 19. Return the number of crime shows with an IMDB rating greater than 7.0.
static int GoodCrimeShows(List<Show> shows)
{
    List<Show> crimeNumber = shows.Where(s => s.Genres.Contains("Crime") && s.ImdbRating > 7).ToList();
    return crimeNumber.Count();
}
...
{
    List<Show> longStanding = shows.Where(s => (s.EndYear - s.StartYear) > 10 && s.ImdbRating < 8).OrderBy(s => s.Name).ToList();
    return longStanding[0];
}

// 21. Return the show with the most words in the name.
static Show WordieastName(List<Show> shows)
{
    List<Show> names = shows.OrderByDescending(s => s.Name.Count(s => s == ' ')).ToList();
    return names.First();
}
...
{
    List<string> spaces = shows.Select(s => s.Name).ToList();
    return spaces.Aggregate((current, next) => current + ", " + next);
}
...
{
    List<string> spaces = shows.Select(s => s.Name).ToList();
    string lastName = spaces.Last();
    spaces.RemoveAt(spaces.Count - 1);
    string combine = spaces.Aggregate((current, next) => current + ", " + lastName);
    string result = combine + " and " + lastName;
    return result;
